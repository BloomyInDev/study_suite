...
export const formatTimeUntil = (target: Date, textBefore = 'dans', from: Date = new Date()): string => {
  ...
  if (target.toLocaleDateString('fr-FR', { timeZone: 'UTC' }) === tomorrow.toLocaleDateString('fr-FR', { timeZone: 'UTC' }))
    return `demain à ${formatTime(target)}`

  return target
    .toLocaleDateString('fr-FR', { weekday: 'long', hour: '2-digit', minute: '2-digit', timeZone: 'UTC' })
    .replace(',', ' à')
}
