...
<template>
  <v-card :color="isCurrent ? 'primary' : undefined">
    ...
    <v-card-text>
      <h3 class="text-h5 font-weight-bold mb-2" :class="isCurrent ? 'text-white text-truncate' : 'text-truncate'">
        {{ event.title }}
      </h3>
      <div class="d-flex align-center mb-1" :class="isCurrent ? 'text-white' : 'text-medium-emphasis'">
        <v-icon size="18" class="mr-2">mdi-clock-outline</v-icon>
        {{ formatTime(event.start) }} - {{ formatTime(event.end) }}
      </div>
      <div v-if="event.rooms.length" class="d-flex align-center" :class="isCurrent ? 'text-white' : 'text-medium-emphasis'">
        <v-icon size="18" class="mr-2">mdi-map-marker</v-icon>
        {{ event.rooms.map(r => r.name).join(', ') }}
      </div>
    </v-card-text>
    <v-card-actions>
      <v-btn :variant="isCurrent ? 'outlined' : 'flat'" :color="isCurrent ? 'white' : 'primary'" to="/planning" append-icon="mdi-arrow-right">
