    ...
    <v-card>
      ...
      <template v-else-if="teacher">
        <v-card-title class="text-h5 pt-4 px-4">
          ...
          <v-btn icon="mdi-close" variant="text" size="small" class="float-right" @click="show = false" />
        </v-card-title>
        <v-card-text class="pa-4">
          <div class="mb-6">
            <v-chip :color="teacher.available ? 'success' : 'error'" class="font-weight-bold">
              {{ teacher.available ? 'Actuellement disponible' : 'Actuellement en cours' }}
            </v-chip>
          </div>
          <div v-if="teacher.currentEvent" class="mb-6">
            ...
            <EventDetailsDialog :event="teacher.currentEvent">
              <template #activator="{ props: aProps }">
                <v-card v-bind="aProps" variant="tonal" color="primary" class="pa-2 cursor-pointer" link>
                  <v-card-item>
                    <template #title>{{ teacher.currentEvent!.title }}</template>
                    <template #subtitle>
                      <div class="d-flex align-center mt-1">
                        <v-icon icon="mdi-map-marker" size="small" class="mr-1" />
                        {{ teacher.currentEvent!.rooms.map(r => r.name).join(', ') }}
                      </div>
                      <div class="d-flex align-center mt-1">
                        <v-icon icon="mdi-clock-outline" size="small" class="mr-1" />
                        {{ formatTime(teacher.currentEvent!.start) }} - {{ formatTime(teacher.currentEvent!.end) }}
                      </div>
                    </template>
                  </v-card-item>
                </v-card>
          ...
          <div>
            ...
            <div v-if="teacher.todayEvents.length > 0">
              <v-list lines="two" density="compact" class="bg-transparent">
                <EventDetailsDialog v-for="event in teacher.todayEvents" :key="event.id" :event="event">
                  <template #activator="{ props: aProps }">
                    <v-list-item v-bind="aProps" class="mb-2 rounded border cursor-pointer" link>
