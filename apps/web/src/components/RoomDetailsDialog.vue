    ...
    <v-card>
      ...
      <template v-else-if="room">
        ...
        <v-card-text class="pa-4">
          <div class="mb-6">
            <v-chip :color="room.available ? 'success' : 'error'" class="font-weight-bold">
          ...
          <div>
            ...
            <div v-if="room.todayEvents.length > 0">
              <v-list lines="two" density="compact" class="bg-transparent">
                <EventDetailsDialog v-for="event in room.todayEvents" :key="event.id" :event="event">
                  <template #activator="{ props: aProps }">
                    <v-list-item v-bind="aProps" class="mb-2 rounded border cursor-pointer" link>
                      ...
                      <v-list-item-subtitle>
                        <span class="mr-3">
                          <v-icon icon="mdi-account-multiple" size="x-small" />
                          {{ event.groups.map(g => g.internalName).join(', ') }}
                        </span>
                        <span>
